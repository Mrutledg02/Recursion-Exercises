...
def dfs(r, c, index, board, word, visited):
    """Depth-first search helper function."""
    if index == len(word):
        return True
    if r < 0 or r >= 5 or c < 0 or c >= 5 or visited[r][c] or board[r][c] != word[index]:
        return False
    ...
    for dr, dc in directions:
        ...
        if dfs(new_row, new_col, index + 1, board, word, visited):
            return True
